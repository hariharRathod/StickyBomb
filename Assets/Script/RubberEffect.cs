...
namespace JellyCube
{
    public class RubberEffect : MonoBehaviour
    {
		...
		public bool reloadAllValuesToDefault;

        [SerializeField]private float m_EffectIntensity = 1;
		[SerializeField]private float m_Damping = 0.7f;
        ...
        void OnValidate()
        {
			//isse comment kiya hai kyuki ye wapas values ko reset karra tha,bhadwa sala.
			if(reloadAllValuesToDefault)
				checkPreset();
        }
        ...
        void Start()
        {
			...
			print("stiffness: " + m_Stiffness);
			print("effect: " + m_EffectIntensity);
